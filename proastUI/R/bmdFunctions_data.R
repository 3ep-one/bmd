...
f.remove.NAs <- function (variableIndices, originalData, track = FALSE) {
	...
	isMissing <- is.na(originalData[, variableIndices])
	rowsMissing <- sum(apply(isMissing, 1, any))
	
	if(any(isMissing)){
		
		warning("There are", rowsMissing, "rows with missing values removed from the data")
